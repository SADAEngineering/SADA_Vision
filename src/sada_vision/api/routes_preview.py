...
async def preview_crack(
    ...
    canvas = draw_overlay(source.rgb, analysis, line_width)
    buffer = io.BytesIO()
    Image.fromarray(canvas).save(buffer, format="PNG", optimize=True)
    return Response(content=buffer.getvalue(), media_type="image/png")


def draw_overlay(rgb: np.ndarray, analysis: Analysis, line_width: int = 0) -> np.ndarray:
...
def _banner(bgr, analysis: Analysis, scale_ui) -> None:
